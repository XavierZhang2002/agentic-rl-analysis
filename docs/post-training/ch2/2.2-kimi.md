...
<figure markdown="span">
  ![Kimi K1.5 大规模 RL 训练系统架构](../../images/post-training/kimi-k15-system-overview.png){ width="70%" }
  <figcaption>Kimi K1.5 RL 训练系统架构：Rollout Workers 采样、Reward Models 评估、Master 调度、Trainer Workers（Policy + Reference Model）训练，通过 Replay Buffer 支持 Partial Rollout（图源：arXiv:2501.12599 Figure 3a）</figcaption>
</figure>
...
<figure markdown="span">
  ![Partial Rollout 机制](../../images/post-training/kimi-k15-partial-rollout.png){ width="65%" }
  <figcaption>Partial Rollout 机制示意：超长轨迹被截断并保存到 Replay Buffer 供后续轮次继续（图源：arXiv:2501.12599 Figure 3b）</figcaption>
</figure>
...
<figure markdown="span">
  ![Kimi K2 Agentic 数据合成 Pipeline](../../images/post-training/kimi-k2-data-synthesis-a.png){ width="75%" }
  <figcaption>Kimi K2 Agentic 数据合成阶段一：从 Domains 出发，通过 MCP tools 和 Applications 构建 Tool Repository（真实 + 合成工具），驱动 Agents 和 Tasks 生成（图源：arXiv:2507.20534 Figure 8a）</figcaption>
</figure>

<figure markdown="span">
  ![Kimi K2 轨迹生成与过滤](../../images/post-training/kimi-k2-data-synthesis-b.png){ width="75%" }
  <figcaption>Kimi K2 Agentic 数据合成阶段二：User Agent 与 Agent 交互，Tool Simulator 执行工具调用，Judge Agent 基于 Rubrics 评估轨迹质量，仅保留高质量数据（图源：arXiv:2507.20534 Figure 8b）</figcaption>
</figure>
