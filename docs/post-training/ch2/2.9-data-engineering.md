...
flowchart TD
    subgraph "V3 路线（单教师）"
        R1["R1 (单一教师)"] -->|"SFT 数据"| V3["V3 Instruct"]
    end
    subgraph "V3.2 路线（多专家）"
        E1["数学专家 (RL)"] --> Merge["合并蒸馏"]
        E2["代码专家 (RL)"] --> Merge
        E3["Agent 专家 (RL)"] --> Merge
        E4["写作专家 (RL)"] --> Merge
        Merge --> V32["V3.2"]
    end
```
...
flowchart TD
    T1["阶段 1: 工具种子 -- 工具库\n3,000 真实 MCP 工具\n层次化领域演化\n20,000+ 合成工具"]
    T2["阶段 2: 工具库 -- Agent + 任务\n为每组工具生成 Agent 定义\nRubric-based 任务生成\n成功标准 + 评估检查点"]
    T3["阶段 3: 任务 -- 训练轨迹\nLLM 模拟用户交互\n工具模拟器执行\nLLM Judge 按 Rubric 过滤"]
    T1 --> T2 --> T3
    T3 -->|"真实沙箱验证"| D["高质量 Agent 训练数据"]
```
...
```mermaid
flowchart LR
    All["全部候选 Query"] -->|"过滤过易/过难"| Mid["中等难度 Query"]
    Mid -->|"在线更新难度"| Curr["课程/优先采样"]
    Curr -->|"动态替换无梯度 batch"| Eff["高效 RL 训练"]
```
...
flowchart TD
    ...
    Neural --> ScalarRM["Scalar RM"]
    Neural --> CoTRM["CoT RM\n(RM 也思考)"]
    Neural --> MultiDim["多维 RM"]

    Self --> SelfReward["Self-Rewarding\n(V3)"]
    Self --> Rubric["Self-Critique Rubric\n(K2)"]
```
...
```mermaid
flowchart LR
    P["竞赛题\n(无公开测例)"] -->|"CYaRon 生成"| T["50 组测例"]
    T -->|"10 份标答提交"| V["交叉验证"]
    V -->|"≥7/10 一致"| OK["有效测例"]
    V -->|"<7/10"| DROP["丢弃"]
    OK -->|"整题 ≥9/10 标答通过"| TRAIN["入训"]
```
...
```mermaid
flowchart TD
    CS["Cold-Start SFT\n少量高质量数据"] -->|"提供格式约束\n缩小搜索空间"| RL1["Reasoning RL\n模型变强"]
    RL1 -->|"RS / 蒸馏"| SFT2["更多 SFT 数据\n从 RL checkpoint 产出"]
    SFT2 -->|"重新 SFT\n常从 Base 重训"| Model2["更强的 SFT 模型"]
    Model2 -->|"可选: 继续 RL"| RL2["General RL\nAgentic RL"]
    RL2 -->|"对齐 + 安全"| Final["最终模型"]
```
