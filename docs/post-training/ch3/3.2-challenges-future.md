---
title: 行业共识与核心挑战
description: 归纳后训练领域六大行业共识与五大核心技术挑战，从算法和工程两个维度进行系统梳理。
tags:
  - 行业共识
  - 核心挑战
---
...
!!! abstract "本节摘要"
    归纳六大行业共识（轻量 SFT + 重度 RL 标准范式、Hard Clipping 原罪、MoE 专用 RL 方案、Query 质量优先、大模型 RL + 小模型蒸馏、工程实现决定成败）与五大核心技术挑战（信任域设计、Critic 规模化困境、不可验证任务奖励、长序列粒度选择、多阶段能力遗忘），所有结论均来自前两章的具体证据。
...
```mermaid
flowchart LR
    ppo([PPO 对称硬裁剪]) --> dapo[DAPO 非对称裁剪]
    dapo --> cispo[CISPO 裁剪 IS 权重]
    dapo --> gspo[GSPO 序列几何平均]
    cispo --> sapo([SAPO Sigmoid 软门控])
    gspo --> sapo
```
...
```mermaid
flowchart TD
    [信任域与硬裁剪] -->|裁剪决定梯度流动| [长序列粒度选择]
    [MoE 路由不稳定] -->|加剧 token IS 波动| [信任域与硬裁剪]
    [MoE 路由不稳定] -->|Critic 路由一致性| [Critic 规模化困境]
    [Critic 规模化困境] -->|Critic 给 token 信号| [长序列粒度选择]
    [不可验证任务奖励] -->|噪声放大不稳定| [信任域与硬裁剪]
    [多阶段能力遗忘] ---|需跨阶段统一优化| [Critic 规模化困境]
```
